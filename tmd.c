...
wup_tmd_body *tmd_get_body(tmd_context *ctx)
{
	...
	wup_tmd_body *body = NULL;

	if (type == TMD_RSA_2048_SHA256 || type == TMD_RSA_2048_SHA1)
	{
		body = (wup_tmd_body*)(ctx->buffer + sizeof(wup_tmd_header_2048));
	}
	else if (type == TMD_RSA_4096_SHA256 || type == TMD_RSA_4096_SHA1)
	{
		body = (wup_tmd_body*)(ctx->buffer + sizeof(wup_tmd_header_4096));
	}
...
{
	unsigned int type = getbe32(ctx->buffer);
	wup_tmd_header_4096* header4096 = 0;
	wup_tmd_header_2048* header2048 = 0;
	wup_tmd_body* body = 0;
	...
	if (type == TMD_RSA_2048_SHA256 || type == TMD_RSA_2048_SHA1)
	{
		header2048 = (wup_tmd_header_2048*)ctx->buffer;
	}
	else if (type == TMD_RSA_4096_SHA256 || type == TMD_RSA_4096_SHA1)
	{
		header4096 = (wup_tmd_header_4096*)ctx->buffer;
	}
	else
	{
		return;
	...
	fprintf(stdout, "Title Category:         %08x\n", getbe32(body->apptype));
	fprintf(stdout, "Unknown:                %08x\n", getbe32(body->unknown));
	fprintf(stdout, "Unknown2:               %08x\n", getbe32(body->unknown2));
	...
	{
		wup_tmd_contentinfo* info = (wup_tmd_contentinfo*)(body->contentinfo + sizeof(wup_tmd_contentinfo)*i);
		...
		memdump(stdout, "Hash:                ", info->hash, 32);
	}
	fprintf(stdout, "\nTMD contents:\n");
